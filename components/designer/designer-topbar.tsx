...
export function DesignerTopbar({
  ...
  const activeSlotLabel = SLOTS_CONFIG.find((s) => s.id === activeSlot)?.label || activeSlot
  ...
  return (
    <>
            ...
            {SLOTS_CONFIG.map((slot) => {
              ...
              return (
                <button
                  ...
                  title={`Editar Superficie: ${slot.label}`}
                >
      ...
      <ConfirmDialog
        ...
        title="¿Restablecer superficie de diseño?"
        description={`Esta acción restaurará la superficie de "${activeSlotLabel}" a su diseño de bloques predeterminado recomendado. Las modificaciones no guardadas se perderán.`}
        confirmText="Restablecer diseño"
        variant="destructive"
        onConfirm={resetCurrentSlotToDefault}
      />

      {/* Confirm Publish Dialog with Global Impact Warning */}
      <ConfirmDialog
        open={isPublishDialogOpen}
        onOpenChange={setIsPublishDialogOpen}
        title="Publicar diseño en producción"
        description="⚠️ Impacto global: Esta acción publicará la plantilla completa del blog en producción y actualizará inmediatamente todas las páginas (Portada, Artículos, Cabecera y Pie) para todos los visitantes."
        confirmText="Publicar en todo el blog"
        variant="default"
        onConfirm={onPublish}
        isLoading={isSaving}
      />
