...
3. ~~**Qué pasa con un artículo generado que el usuario abandona**~~ → **DECIDIDO:** retención de 90 días, y la purga borra **solo sesiones en estado terminal**. Una sesión activa no se borra sola por vieja que sea. El post generado no se toca: es del usuario y vive por su cuenta. La mutation `purgeExpiredSessions` está implementada, pero **el cron que la dispara NO se incluyó**: un job diario que borra datos no entra a una aplicación que ya está en producción antes de que Composer exista y haya algo que purgar. Registrarlo en `convex/crons.ts` es el último paso de #20, cuando el feature flag se encienda.
